...
The Windows clipboard is a temporary area used to store data that a user copies, such as:
...
Clipboard access is normally legitimate. The DFIR challenge is determining whether a process is reading clipboard contents in a suspicious context.

A simple attack scenario is:

User
  ↓
Copies sensitive information
  ↓
Data enters Windows Clipboard
  ↓
Suspicious application reads Clipboard
  ↓
Data is stored or transmitted

The important point is that copying sensitive data is not itself proof of data theft. We need evidence showing that another process accessed, collected, stored, or transmitted the clipboard contents.

We'll create a harmless controlled clipboard scenario, rather than using real passwords, tokens, or sensitive information.

We'll investigate:

What information was placed into the clipboard.
Which process accessed the clipboard.
Whether suspicious processes were running around the same time.
Whether the process created files containing clipboard data.
Whether network activity followed the clipboard access.
Whether Windows telemetry can provide supporting evidence.
Whether the activity can actually be attributed to data theft.
...
The investigation was designed to determine whether clipboard-related activity could be reconstructed from the available Windows evidence and whether the observed activity showed signs of unauthorized data collection.

The main objectives were:

Determine how clipboard contents can be accessed by a running process.
Identify the process responsible for accessing the clipboard.
Establish when the clipboard-related activity occurred.
Determine whether the clipboard contents were subsequently stored on disk.
Examine the relationship between process execution and file creation.
Review available network activity around the same timeframe.
Determine whether the collected data remained local or showed evidence of transmission.
Compare process, file, and network activity to identify a connected sequence of events.
Distinguish legitimate clipboard usage from behavior that could indicate unauthorized collection.
Identify gaps in the available telemetry that could prevent a definitive conclusion.
Determine what evidence would be required to confidently classify the activity as clipboard-based data theft.
